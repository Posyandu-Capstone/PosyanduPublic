                ...
                <form class="form-grid" @submit.prevent="handleSubmit">
                    ...
                    <div class="section">
                        ...
                        <div class="form-row">
                            ...
                            <select v-model="formData.jenisKelamin">
...
export default {
    ...
    methods: {
        handleSubmit() {
            ...
            const data = {
                nama_lengkap: this.formData.namaLengkap,
                jenis_kelamin: this.formData.jenisKelamin,
                tanggal_lahir: this.formatTanggal(this.formData.tanggalLahir),
                no_kk: this.formData.noKK,
                nik: this.formData.nik,
                anak_ke: this.formData.anakKe,
                tinggi_lahir: parseInt(this.formData.tinggiLahir) || null,
                berat_lahir: this.formData.beratLahir.toString() || "",
                penyakit_bawaan: this.formData.penyakitBawaan,
                nama_ayah: this.formData.namaAyah,
                nik_ayah: this.formData.nikAyah,
                nama_ibu: this.formData.namaIbu,
                nik_ibu: this.formData.nikIbu,
                nomor_telp: this.formData.noHP,
                nama_dusun: this.formData.dusun,
                alamat: this.formData.rtrw,
                bpjs: this.formData.bpjs,
                gakin: this.formData.gakin,
            };
                ...
                .catch((error) => {
                    if (error.response) {
                        if (error.response.data.errors) {
                            ...
                            alert(
                                "Terdapat kesalahan pada form. Cek detail di console."
                            );
                        } else {
                            console.error(
                                "Response API Error:",
        ...
        validateForm() {
            const requiredFields = [
                { key: "namaLengkap", message: "Nama Lengkap wajib diisi" },

                { key: "nik", message: "NIK harus terdiri dari 16 digit!" },
                { key: "noKK", message: "No KK harus terdiri dari 16 digit!" },

                { key: "jenisKelamin", message: "Jenis Kelamin wajib diisi!" },
                {
                    key: "beratLahir",
                    message: "Berat lahir wajib diisi dan berupa angka!",
                },
                { key: "tanggalLahir", message: "Tanggal lahir wajib diisi!" },
            ];

            for (const field of requiredFields) {
                const value = this.formData[field.key];
                if (!value || (field.key === "nik" && value.length !== 16)) {
                    alert(field.message);
                    return false;
                } else if (
                    !value ||
                    (field.key === "no_kk" && value.length !== 16)
                ) {
                    alert(field.message);
                    return false;
                }
            ...
            if (isNaN(this.formData.beratLahir)) {
                alert("Berat lahir harus berupa angka!");
                return false;
            }
